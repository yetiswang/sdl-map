...
공공 부문이 주도하거나 여러 기관에 걸친 국가/지역 규모의 SDL 프로젝트. NSF PCL Test Bed, Acceleration Consortium, BIG-MAP / FULL-MAP, PEPR DIADEM, DiscoveryLabNL 등 26개 전체 항목은 [English README → National Programmes & Consortia](README.md#-national-programmes--consortia)를 참고하세요.

## 🎓 학술 그룹

단일 기관 또는 PI가 주도해 실제로 SDL을 운영하는 연구 그룹. A-Lab, Matter Lab, ODACell, Cooper Group, Jensen Lab, Jun Jiang 그룹 / ChemAgents + Robot Chemist 등 48개 전체 항목은 [English README → Academic Groups](README.md#-academic-groups)를 참고하세요.
...
정정, 추가, 이름 변경 / 종료 보고를 환영합니다. Issue나 PR을 보내주세요. Git이 익숙하지 않다면 **[제출 양식](https://github.com/yetiswang/sdl-map/issues/new?template=add-lab.yml)**(구조화된 템플릿에 항목만 입력)을 이용하셔도 됩니다. 신규 항목에는 다음을 포함해 주세요:
