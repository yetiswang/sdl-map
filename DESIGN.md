...
- **Controls, desktop:** one vocabulary — 36 px, `--r-soft`, `--surface-overlay`, 1 px `--rule`, no shadow — for the top-left row (about · clear · export · data at a 44 px pitch), the zoom column and the region buttons (bottom-right). `public/map-chrome.css` owns this; it must out-specify the styles export.js and map-i18n.js inject at runtime.
